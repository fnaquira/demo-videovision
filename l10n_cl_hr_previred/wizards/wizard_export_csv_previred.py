...
class WizardExportCsvPrevired(models.TransientModel):
    ...
    def get_regimen_provisional(self, contract): # -----
        ...
        # else:
        #     return 'AFP'
        return contract.l10n_cl_pension_system if contract.l10n_cl_pension_system else 'SIP'
    ...
    def get_tipo_trabajador(self, employee): # -----
        ...
        return employee.contract_id.l10n_cl_worker_type if employee.contract_id.l10n_cl_worker_type else '0'
    ...
    def get_payslip_lines_value_2(self, obj, regla):
        ...
        valor = detalle.amount if detalle else 0
        return valor        
        ...
        for payslip in payslip_recs:
            ...
            line_employee = [self._acortar_str(rut, 11), 
                            ...
                            #  self._arregla_str(payslip.employee_id.mothers_name.upper(), 30)  if payslip.employee_id.mothers_name else "",
                             self._arregla_str(payslip.employee_id.mother_last_name.upper(), 30) if payslip.employee_id.mother_last_name else "",
                            #  "%s %s" % (self._arregla_str(payslip.employee_id.firstname.upper(), 15), self._arregla_str(payslip.employee_id.middle_name.upper(), 15) if payslip.employee_id.middle_name else ''),
                             "%s" % (self._arregla_str(payslip.employee_id.first_name.upper(), 30) if payslip.employee_id.first_name else ""),
                             sexo_data.get(payslip.employee_id.gender, "") if payslip.employee_id.gender else "",
                             self.get_nacionalidad(payslip.employee_id.country_id.id) if payslip.employee_id.country_id else "0",
                             ...
                             #15
                             '0', # payslip.movimientos_personal, -----
                             ...
                             '00/00/0000', # payslip.date_from.strftime("%d/%m/%Y") if payslip.movimientos_personal != '0' else '00/00/0000', -----
                             #payslip.date_from if payslip.date_from else '00/00/0000', 
                             #17 Fecha fin movimiento personal (dia-mes-año)
                             '00/00/0000', # payslip.date_to.strftime("%d/%m/%Y") if payslip.movimientos_personal != '0' else '00/00/0000', -----
                             #Si declara mov. personal 1, 3, 4, 5, 6, 7, 8 y 11 Fecha Desde
                             #es obligatoria y debe estar dentro del periodo de remun
                             ...
                             #19 NCargas Simples
                             '0', # payslip.contract_id.carga_familiar, -----
                             '0', # payslip.contract_id.carga_familiar_maternal, -----
                             '0', # payslip.contract_id.carga_familiar_invalida, -----
                             ...
                             payslip.contract_id.afp_id.codigo if bool(payslip.contract_id.afp_id) else '00',
                             #27
...
#caso se paga en la mutual respectiva. Datos no numérico 
                             ...
                             payslip.contract_id.isapre_id.codigo if bool(payslip.contract_id.isapre_id) else '00',
                             #76 Numero del FUN
                            #  " " if payslip.contract_id.isapre_id.codigo=='07' else payslip.contract_id.isapre_fun if payslip.contract_id.isapre_fun else "", # -----
                             " " if payslip.contract_id.isapre_id.codigo=='07' else payslip.contract_id.l10n_cl_hr_isapre_fun if payslip.contract_id.l10n_cl_hr_isapre_fun else "",
                            ...
                            #  "1" if payslip.contract_id.isapre_id.codigo=='07' else "2", # -----
                             "1" if bool(payslip.contract_id.isapre_id) and payslip.contract_id.isapre_id.codigo=='07' else "2",
                            ...
                            #  "0" if payslip.contract_id.isapre_id.codigo=='07' else payslip.contract_id.isapre_cotizacion_uf, # -----
                             "0" if bool(payslip.contract_id.isapre_id) and payslip.contract_id.isapre_id.codigo=='07' else payslip.contract_id.l10n_cl_hr_isapre_cotizacion_uf,
                            ...
                            #  "0" if payslip.contract_id.isapre_id.codigo=='07' else int(self.get_payslip_lines_value_2(payslip,'SALUD')), # -----
                             "0" if bool(payslip.contract_id.isapre_id) and payslip.contract_id.isapre_id.codigo=='07' else int(self.get_payslip_lines_value_2(payslip,'SALUD')),
                            ...
                            #  "0" if payslip.contract_id.isapre_id.codigo=='07' else int(self.get_payslip_lines_value_2(payslip,'ADISA')), # -----
                             "0" if bool(payslip.contract_id.isapre_id) and payslip.contract_id.isapre_id.codigo=='07' else int(self.get_payslip_lines_value_2(payslip,'ADISA')),
